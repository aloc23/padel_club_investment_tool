...
function calculateGym() {
  ...
  if (document.getElementById('gymRamp').checked) {
    ...
    totalAnnualRevenue = rampedRevenue / 12 * 12; // effectively totalAnnualRevenue
  }
...
function updatePnL() {
  ...
  updateMonthlyBreakdown(plType, padel, gym);
}

function updateMonthlyBreakdown(plType, padel, gym) {
  const tbody = document.querySelector('#monthlyBreakdown tbody');
  tbody.innerHTML = '';
  if(plType !== 'monthly') return;

  for(let i=1; i<=12; i++) {
    const rev = (padel.monthlyRevenue + gym.monthlyRevenue).toFixed(2);
    const costs = (padel.monthlyCosts + gym.monthlyCosts).toFixed(2);
    const profit = (padel.monthlyProfit + gym.monthlyProfit).toFixed(2);
    const tr = document.createElement('tr');
    tr.innerHTML = `<td>Month ${i}</td><td>€${rev}</td><td>€${costs}</td><td>€${profit}</td>`;
    tbody.appendChild(tr);
  }
}

function updateROI() {
  const padel = window.padelData || { monthlyProfit: 0 };
  const gym = window.gymData || { monthlyProfit: 0 };
