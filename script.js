...
window.onload = () => {
  ...
  if (typeof showTab === "function") showTab('padel');
  if (typeof calculatePadel === "function") calculatePadel();
  if (typeof calculateGym === "function") calculateGym();
...
function updatePnL() {
  ...
  document.getElementById("pnlRevenue").textContent = "€" + revenue.toFixed(2);
  document.getElementById("pnlCosts").textContent = "€" + costs.toFixed(2);
  document.getElementById("pnlProfit").textContent = "€" + profit.toFixed(2);
...
function updateROI() {
  ...
  window.roiChart = new Chart(ctx, {
    ...
    options: {
      ...
      plugins: {
        legend: { display: false },
        title: { display: true, text: 'ROI Breakdown' }
      }
    }
  });
}
